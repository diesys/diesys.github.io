...
// FuzzyText from react-bits.dev, adapted for this site:
// - The text comes via a `text` prop, not children: inside an Astro island the
//   children arrive as a React element, so Children.toArray(...).join('') would
//   render as "[object Object]".
// - Renders on a 2D canvas, so the heading color can't be a CSS var(): it is
//   resolved from the --ink token at init and re-rendered when the .dark theme
//   class changes (MutationObserver, same pattern as LetterGlitch/WarpText).
// - The canvas carries no text for assistive tech/SEO: use it inside an element
//   with a visually-hidden text alternative (the 404 page does this).
...
const FuzzyText = ({
  ...
  // The .dark class swaps the design tokens; the canvas text is rasterized with
  // its own fill color, so re-run the effect (re-render the text) on theme swap.
  useEffect(() => {
  ...
  useEffect(() => {
    // themeVersion is only a re-run trigger: bumping it (theme change) re-runs
    // this effect so the canvas text re-rasterizes with the new --ink value.
    void themeVersion;
    ...
    const init = async () => {
      ...
      // falling back to the light-theme ink value.
      const resolvedColor =
        color ||
